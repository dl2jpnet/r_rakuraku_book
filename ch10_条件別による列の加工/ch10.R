...
logvec <- c(TRUE, FALSE, TRUE, FALSE, FALSE)
...
if_else(c(TRUE, FALSE, NA), "□", "■")

# missing引数を設定すると別のものに置き換えることができる
if_else(c(TRUE, FALSE, NA), "□", "■", missing = "--ないよ--")

# ただし、なぜかNAで置き換えようとするとエラーがでる
if_else(c(TRUE, FALSE, NA), "□", "■", NA)
...
#NAを設定したい場合
if_else(c(TRUE, FALSE, NA), "□", "■", NA) #エラー
if_else(c(TRUE, FALSE, NA), "□", "■", NA_character_) #OK

#missing以外にも設定できる
if_else(c(TRUE, FALSE, NA), 1000, NA_real_, 99999)
