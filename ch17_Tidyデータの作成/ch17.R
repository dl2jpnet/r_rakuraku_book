...
dat %>%
  fill(nen, tuki, tenpo, staff_id) %>%
  pivot_longer(cols = matches("\\d+"), names_to = "niti",
               values_to = "time") %>%
  mutate(hiduke = make_date(nen, tuki, niti)) %>%
  select(hiduke, tenpo, staff_id, kintai, time) %>%
  filter(!is.na(hiduke)) %>%
...
# Declare global variables to avoid no visible binding error
utils::globalVariables(c("味", "nen", "tuki", "tenpo", "staff_id",
                         "kintai", "time", "hiduke", "start",
                         "end", "ranking", "aji", "month_diff",
                         "kongetu", "nendo", "tenpo", "sex",
                         "age", "value", "味"))
